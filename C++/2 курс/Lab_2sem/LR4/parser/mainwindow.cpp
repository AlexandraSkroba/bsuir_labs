...
void MainWindow::getVarsData()
{
    ...
    std::regex regular("^[0-31]\\.[0-9]{2}\\s(Mon|Tue|Wed|Thu|Fri|Sat|Sun)\\s[0-23]:[0-59]$");
    std::smatch result;
...
void MainWindow::showVarsData()
{
    ...
    for(auto& it : date1)
    {
        if(it.find("Mon") != std::string::npos)
        { ++int_count; }
        if(it.find("Tue") != std::string::npos)
        { ++float_count; }
        if(it.find("Wed") != std::string::npos)
        { ++double_count; }
        if(it.find("Thu") != std::string::npos)
        { ++char_count; }
        if(it.find("Fri") != std::string::npos)
        { ++size_t_count; }
