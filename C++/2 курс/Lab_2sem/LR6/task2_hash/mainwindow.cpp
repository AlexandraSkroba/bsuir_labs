    ...
    for(int i=0;i<table.vector.size();i++){
        if(!this->table.vector[i].isEmpty()){
            for(int j=0;j<this->table.vector[i].getSize();j++){
                ...
                QString t, p;
                ui->tableWidget->setItem(rows,0, new QTableWidgetItem(t.setNum(this->table.vector[i][j].key)));
                ui->tableWidget->setItem(rows,1,new QTableWidgetItem(this->table.vector[i][j].data));
                rows++;
            }
        }
    }
